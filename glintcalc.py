...
class glintcalc:
    ...
    def get_null_vals_MC(self, astro_null, deltaphi_sig, deltaI_sig, deltaphi_mu=0, deltaI_mu=0,
        ...
        self.nullsamps = 0.25 * (dIsamps**2 + dphisamps**2) + astro_null
    ...
    def plot_null_dphi(self, astro_null, deltaI_sig, max_dphi=None, npoints=100):
        if max_dphi is None:
            max_dphi = self.wavelength/10
        dphis = np.linspace(0, max_dphi, npoints)
        all_nulls = np.zeros(npoints)
        for k in range(npoints):
            all_nulls[k] = self.get_null_vals_MC(
                astro_null, dphis[k], deltaI_sig)
        plt.figure(2)
        plt.clf()
    ...
    def get_chromatic_null(self, astro_null, deltaphi_sig, deltaI_sig, bandwidth, npoints=50, show_plot=False):
        ...
        all_nulls = np.zeros(npoints)
        for k in range(npoints):
            all_nulls[k] = self.get_null_vals_MC(astro_null, deltaphi_sig, deltaI_sig, deltaphi_mu=all_wl_offsets[k],
                                                 num_samps=1000000)
    ...
    def get_snr(self, photon_flux, bandwidth, contrast, null_depth, throughput=1, pupil_area=50,
        ...
        read_noise_tot = read_noise * np.sqrt(num_pix)
        ...
        raw_comp_snr = companion_flux / \
            np.sqrt(star_photons + read_noise_tot**2)
        print('No-nulling S/N ratio for companion: %f' % raw_comp_snr)

        nulled_comp_snr = companion_flux / \
            np.sqrt(star_photons*null_depth + read_noise_tot**2)
...
def get_injection(diam, r0, order=1, geo_inj=0.8, wl=None, wfe=None):
    ...
    if not wl is None and not wfe is None:
        ...
        # Get variance of the phase residuals after correction of the first N order of Zernike.
        residuals = get_noll_residuals(diam, r0, order)
    strehl = np.exp(-residuals)  # Marechal's approximation

    # Lower bound of strehl. Source: Olivier Guyon's mail 2020-09-23 4:17 am
    low_sr = 1 - 2 * (1 - strehl)
    # Previous relationship can give negative strehl, set a threshold at 0
    low_sr = max(low_sr, 0)
    total_range = 1 - low_sr
    # We assume a Normal distribution of the injection which is what we observe
    std_sr = total_range / 6
...
def get_diff_piston(wfe, n_actuator):
    ...
    # Std diff piston = sqrt(std piston 1 + std piston 2), with 1 and 2 the pupils
    return piston_rms * 2**0.5
